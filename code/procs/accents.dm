...
/proc/wonk_parse(var/string)
	...
	if(prob(1))
		return pick("yiff yiff mrr", "fuckable owwwwwwls", "cunts", "shit cunt")

	var/list/broken_string = splittext(string, " ")
	for(var/i = 1; i <= broken_string.len;i++)
		if(prob(20))
			broken_string[i] = pick("cunt", "shit", "horsefucker", "prick", "yiff", "mrr", "bubs' mum", "fisting", "fuck", "cunting", "chode", "shitehawk")
